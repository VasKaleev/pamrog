...
<body>
    ...
    <section>
        ...
        <h4 id="zag_comment">для проведения консультации. Ввод номера</h4>
        <h4 id="zag_comment">телефона для обратной связи обязателен.</h4>
        ...
        if (!empty($_POST['fio'])) {
            ...
            $to = 'monolit.gran@mail.ru';
            // адрес получателя
            $subject = 'Пользователь на сайте памятники в Рогачеве оставил заявку на консультацию.';
            // текст письма
            $message = 'ФИО: ' . $fio . '<br/>';
            ...
            if (isset($_POST['fio']) and isset($_POST['tel'])) {
                ...
                if (isset($_POST['fio'])) {
                    ...
                    $sql = mysqli_query($db, "INSERT INTO `zayav` (`fio`, `tel`, `comment`, `datazv`) VALUES ('{$_POST['fio']}', '{$_POST['tel']}', '{$_POST['comment']}', '{$datazv}')");
        ...
        ?>
        <hr />
        <input id="btnAdmin" type='submit' value='Личный кабинет администратора сайта' />
    </section>
    <footer>
